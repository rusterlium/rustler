...
use super::nif_interface::{ NIF_ENV, NIF_TERM, ERL_NIF_BIN2TERM_SAFE };
...
pub unsafe fn binary_to_term(env: NIF_ENV, data: &[u8], safe: bool) -> Option<(NIF_TERM, usize)> {
    ...
    let read_count = nif_interface::enif_binary_to_term(env, data.as_ptr(), data.len(), &mut result, opts);
