...
use rustler::{NifEnv, Term, Encoder, NifResult, NifError};
use rustler::types::binary::{ NifBinary, OwnedNifBinary };

pub fn make_shorter_subbinary<'a>(env: NifEnv<'a>, args: &[Term<'a>]) -> NifResult<Term<'a>> {
...
pub fn parse_integer<'a>(env: NifEnv<'a>, args: &[Term<'a>]) -> NifResult<Term<'a>> {
...
pub fn binary_new<'a>(env: NifEnv<'a>, _args: &[Term<'a>]) -> NifResult<Term<'a>> {
    let mut binary = OwnedNifBinary::new(4).unwrap();
    binary.as_mut_slice().write(&[1, 2, 3, 4]).unwrap();
    Ok(binary.release(env).encode(env))
}

pub fn unowned_to_owned<'a>(env: NifEnv<'a>, args: &[Term<'a>]) -> NifResult<Term<'a>> {
...
pub fn realloc_shrink<'a>(env: NifEnv<'a>, _args: &[Term<'a>]) -> NifResult<Term<'a>> {
...
pub fn realloc_grow<'a>(env: NifEnv<'a>, _args: &[Term<'a>]) -> NifResult<Term<'a>> {
    let mut binary = OwnedNifBinary::new(4).unwrap();
    binary.as_mut_slice().write(&[1, 2, 3, 4]).unwrap();
