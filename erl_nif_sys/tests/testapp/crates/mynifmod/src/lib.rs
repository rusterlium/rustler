...
unsafe fn rustmap_dtor_count(
    env: *mut ErlNifEnv,
    _: c_int,
    _: *const ERL_NIF_TERM,
) -> ERL_NIF_TERM {
...
unsafe fn to_str(env: *mut ErlNifEnv, args: &[ERL_NIF_TERM]) -> ERL_NIF_TERM {
    ...
    if n < 0 {
        ...
        buf.set_len(len);
        enif_make_string_len(env, buf.as_ptr(), len, ErlNifCharEncoding::ERL_NIF_LATIN1)
    }
}
...
unsafe fn make_map(env: *mut ErlNifEnv, args: &[ERL_NIF_TERM]) -> ERL_NIF_TERM {
    if 0 == args.len() {
        let keys: Vec<_> = ["one", "two", "three"]
            ...
            .collect();
        let values: Vec<_> = (1..=3).map(|x| enif_make_int(env, x)).collect();
